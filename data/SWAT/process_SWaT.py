...
df_test = to_numeric_columns(df_test)
...
df_test.to_csv('SWATv0_test.csv', index=False, header=True)  # Ensure header is written
...
def check_non_numeric(df, name):
  for column in df.columns:
    if df[column].dtype == 'object' and column != 'Timestamp':
      print(f"Column {column} in {name} data contains non-numeric values")
    elif df[column].isnull().any():
      print(f"Column {column} in {name} data contains NaN values")
   
print("\nChecking train data:")
check_non_numeric(df, 'train')
print("\nChecking test data:")
check_non_numeric(df_test, 'test')