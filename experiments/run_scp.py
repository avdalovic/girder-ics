...
class SCPPredictor:
    """
    Standard Split Conformal Prediction (SCP) baseline.
    
    Uses calibration residuals to compute empirical quantiles for prediction intervals.
    """
    
    def __init__(self, alphas: List[float], sensor_names: List[str] = None):
        self.alphas = sorted(alphas)
        self.sensor_names = sensor_names
        self.calibration_residuals = None
        self.quantile_thresholds = {}
        
    def fit(self, calibration_residuals: np.ndarray):
        """
        Fit SCP using calibration residuals.
        
        Args:
            calibration_residuals: [N_calib, N_sensors] array of residuals
        """
        self.calibration_residuals = calibration_residuals
        
        # Compute quantile thresholds for each alpha
        for alpha in self.alphas:
            # For SCP, we use the empirical quantile of absolute residuals
            
            abs_residuals = np.abs(calibration_residuals)     # [N_calib, N_sensors]
            n = abs_residuals.shape[0]
            q_level = np.ceil((n + 1) * (1 - alpha)) / n
            threshold = np.quantile(abs_residuals, q_level, axis=0)  # <-- per-sensor
            self.quantile_thresholds[alpha] = threshold               # shape: [N_sensors]
    
    def predict_intervals(self, predictions: np.ndarray) -> Dict[float, Tuple[np.ndarray, np.ndarray]]:
        """
        Generate prediction intervals using SCP.
        ...
        for alpha in self.alphas:
            threshold = self.quantile_thresholds[alpha]  # [N_sensors]
...
def run_experiment(cfg: DictConfig):
    ...
    # Split calibration into train/val for SCP
    ...
    print("Implementing Standard Split Conformal Prediction (SCP)")
    
    # Initialize SCP predictor
    scp_predictor = SCPPredictor(
        alphas=cfg.alphas,
    ...
    print(f"Warming up SCP with {len(calib_data)} calibration samples")
    
    # FIXED: Build calibration residuals DIRECTLY from the saved DataFrame
    ...
    # Fit SCP
    scp_predictor.fit(calib_residuals)
    
    ########################################
    # Evaluation                           #
    ########################################
    
    print("Evaluating SCP on test set")
    ...
    # Residuals for the SAME test indices (scaled)
    ...
    residuals_test_scaled = residuals_all[test_mask]  # [N_test, N_sensors]

    # Base predictions (scaled)
    yhat_scaled = y_scaled - residuals_test_scaled
    
    print(f"True targets shape: {y_scaled.shape}")
    print(f"Residuals shape: {residuals_test_scaled.shape}")
    ...
    for alpha in cfg.alphas:
        q_scaled = scp_predictor.quantile_thresholds[alpha]

        lower_scaled = yhat_scaled - q_scaled
        upper_scaled = yhat_scaled + q_scaled

        if cfg.dataset.name == 'tep':
            
            yhat_original = yhat_scaled
            y_original = y_scaled
            lower_orig = lower_scaled
            upper_orig = upper_scaled
        else:
            #Use sklearn scaler for original metrics
        ...
        if alpha == 0.1:  # 90% CI results only
            ...
            print(f"  PI Width (Original): {pi_width:.4f}")
            print(f"  Winkler Score (Original): {winkler:.4f}")
            if cfg.dataset.name != 'tep':
                print(f"  PI Width (Scaled): {pi_width_original:.4f}")
                print(f"  Winkler Score (Scaled): {winkler_original:.4f}")
    ...
    if cfg.get('use_wandb', False) and WANDB_AVAILABLE:
        wandb_logger = WandbLogger(
            ...
            name=f"{cfg.dataset.name}_scp_baseline",
            tags=cfg.wandb.get('tags', ['scp', 'baseline', cfg.dataset.name]),
            notes=cfg.wandb.get('notes', 'SCP baseline evaluation'),
    ...
    results_path = os.path.join(cfg.run.dir, 'scp_results.csv')
    results_df.to_csv(results_path, index=False)
    print(f"Results saved to {results_path}")
    
    print("SCP evaluation completed successfully!")
    return results
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run SCP baseline evaluation')
    ...
    parser.add_argument('--run_dir', type=str, default='logs/baseline/scp',
                        help='Directory to save results')
    parser.add_argument('--run_name', type=str, default='scp_evaluation',
    ...
    cfg = DictConfig({
        ...
        'dataset': {
            'name': dataset_name  # Use detected dataset name
        }
    })
